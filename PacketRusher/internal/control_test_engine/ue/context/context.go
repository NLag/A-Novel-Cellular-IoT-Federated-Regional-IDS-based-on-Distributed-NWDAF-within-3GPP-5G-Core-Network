...
func (pduSession *UEPDUSession) SetTunnelCleanup(cleanup func()) {
	pduSession.cleanupOnce = sync.Once{}
	pduSession.tunnelCleanup = cleanup
}

func (pduSession *UEPDUSession) CleanupTunnel() bool {
	cleaned := false
	pduSession.cleanupOnce.Do(func() {
		if pduSession.tunnelCleanup != nil {
			pduSession.tunnelCleanup()
			cleaned = true
		}
	})
	return cleaned
