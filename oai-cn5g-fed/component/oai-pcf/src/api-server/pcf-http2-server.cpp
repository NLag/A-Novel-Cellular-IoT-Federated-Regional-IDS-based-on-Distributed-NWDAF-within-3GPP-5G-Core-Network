...
void pcf_http2_server::start() {
  ...
  server.handle(
      sm_policies::API_BASE + pcf_cfg->local().get_sbi().get_api_version() +
          "/ids-traffic-influence",
      [&](const request& request, const response& response) {
        ...
        if (request.method() == "GET") {
          std::string body =
              m_smpc_service->ids_traffic_influence_status_handler();
          header_map h_map;
          h_map.emplace(
              "content-type", header_value{"application/json", false});
          response.write_head(
              static_cast<unsigned int>(http_status_code::OK), h_map);
          response.end(body);
          return;
        }
        ...
        request.on_data([&, request_body](const uint8_t* data,
                                           std::size_t len) {
          if (len > 0) {
            std::copy(
                data, data + len,
          ...
          try {
            auto payload = nlohmann::json::parse(request_body->str());
            ...
            std::string body =
                m_smpc_service->ids_traffic_influence_handler(active);
            header_map h_map;
            h_map.emplace(
                "content-type", header_value{"application/json", false});
            response.write_head(
