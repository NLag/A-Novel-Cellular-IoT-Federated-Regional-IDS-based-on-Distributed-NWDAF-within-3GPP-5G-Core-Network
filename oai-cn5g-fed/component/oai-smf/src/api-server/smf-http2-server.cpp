  ...
  server.handle(
      ...
      [&](const request& request, const response& response) {
        ...
        request.on_data([&, request_body](const uint8_t* data,
          ...
          try {
            const auto payload = nlohmann::json::parse(request_body->str());
            if (payload.contains("smPolicyDecision")) {
              const auto decision =
                  payload.at("smPolicyDecision")
                      .get<oai::model::pcf::SmPolicyDecision>();
              m_smf_app->handle_ids_traffic_influence_notification(
                  decision);
            }
            ...
            Logger::smf_sbi().warn(
                "Can not parse IDS policy notification (error: %s)!",
                e.what());
            response.write_head(http_status_code::BAD_REQUEST);
            response.end();
            return;
