...
namespace {
bool ids_pcf_probe_enabled() {
  const char* enabled = std::getenv("OAI_SMF_IDS_PCF_PROBE_ENABLED");
  if (!enabled) {
    return false;
  }
...
bool sm_policy_decision_has_ids_duplication_rule(
  ...
  for (const auto& rule : decision.getPccRules()) {
    if (rule.first == "ids-duplication" ||
        rule.second.getPccRuleId() == "ids-duplication") {
      return true;
    }
  }
...
uint32_t smf_n7::select_pcf(const SmPolicyContextData& context) {
  ...
  if (policy_storages.empty()) {
    ...
    // client
    if (smf_cfg->use_local_pcc_rules && !ids_pcf_probe_enabled()) {
      Logger::smf_n7().warn(
          "Local PCC rules are enabled and IDS PCF probe is disabled");
      return 0;
    } else {
      PlmnId plmn_id = {};
...
void smf_n7::probe_pcf_for_ids_notifications() {
  if (!ids_pcf_probe_enabled()) {
    Logger::smf_n7().info("IDS PCF startup probe is disabled");
    return;
  }
  if (m_ids_pcf_startup_probe_done.exchange(true)) {
    return;
  }
  ...
  association.set_context(
      "001010000000000", "imsi-", "oai", snssai, plmn, 1, pdu_session_type);
  association.context.setNotificationUri(
      smf_cfg->local().get_sbi().get_url() +
      "/npcf-smpolicycontrol/v1/ids-notify");

  Logger::smf_n7().info(
      "Probing PCF once at SMF startup for IDS traffic influence "
      "notifications at %s",
      association.context.getNotificationUri().c_str());

  const auto status = create_sm_policy_association(association);
  if (status != sm_policy_status_code::CREATED) {
    Logger::smf_n7().warn(
        "IDS PCF startup probe failed; SMF will keep local PCC/default rules");
    return;
  }

  set_ids_traffic_influence_active(
      sm_policy_decision_has_ids_duplication_rule(association.decision));
  Logger::smf_n7().info(
      "IDS PCF startup probe completed; SMF is waiting passively for PCF "
      "notifications");
}

void smf_n7::set_ids_traffic_influence_active(bool active) {
  m_ids_traffic_influence_active.store(active);
  Logger::smf_n7().info(
      "IDS traffic influence state set to %s", active ? "active" : "inactive");
}

bool smf_n7::ids_traffic_influence_active() const {
  return m_ids_traffic_influence_active.load();
}
...
std::shared_ptr<smf_pcf_client> smf_pcf_client::discover_pcf(
  ...
  if (smf_cfg->use_local_pcc_rules && !ids_pcf_probe_enabled()) {
    Logger::smf_n7().info(
        "Local PCC rules are enabled and IDS PCF probe is disabled, do not "
        "discover PCF");
    return nullptr;
  }
