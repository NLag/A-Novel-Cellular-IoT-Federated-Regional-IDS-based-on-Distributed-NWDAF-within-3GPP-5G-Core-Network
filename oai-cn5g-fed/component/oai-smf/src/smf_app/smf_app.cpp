...
bool sm_policy_decision_has_ids_duplication_trigger(
  ...
  for (const auto& rule : decision.getPccRules()) {
    ...
    if (!ids_rule || !pcc_rule.refTcDataIsSet() ||
        !decision.traffContDecsIsSet()) {
      continue;
    }

    const auto traffic_controls = decision.getTraffContDecs();
    for (const auto& traffic_control_id : pcc_rule.getRefTcData()) {
      const auto traffic_control = traffic_controls.find(traffic_control_id);
      if (traffic_control != traffic_controls.end() &&
          traffic_control->second.traffCorreIndIsSet() &&
          traffic_control->second.isTraffCorreInd()) {
        return true;
      }
    }
  }
...
smf_app::smf_app(const std::string& config_file)
  ...
  apply_config();
  n7::smf_n7::get_instance().probe_pcf_for_ids_notifications();
...
//------------------------------------------------------------------------------
void smf_app::handle_ids_traffic_influence_notification(
    const oai::model::pcf::SmPolicyDecision& decision) {
  const bool active = sm_policy_decision_has_ids_duplication_trigger(decision);
  n7::smf_n7::get_instance().set_ids_traffic_influence_active(active);

  std::vector<std::pair<scid_t, supi64_t>> session_refs;
  {
    std::shared_lock lock(m_scid2smf_context);
    for (const auto& context : scid2smf_context) {
      if (!context.second) {
        continue;
      }
      session_refs.emplace_back(
          context.first, smf_supi_to_u64(context.second->supi));
    }
  }

  uint32_t updated_sessions = 0;
  for (const auto& session_ref : session_refs) {
    std::shared_ptr<smf_pdu_session> sp = {};
    if (!find_pdu_session(session_ref.first, sp) || !sp) {
      continue;
    }

    sp->ids_traffic_duplication_enabled = active;
    if (!active) {
      sp->ids_traffic_duplication_host = {};
      sp->ids_traffic_duplication_port = 0;
      sp->ids_traffic_duplication_teid = 0;
      updated_sessions++;
      continue;
    }

    sp->ids_traffic_duplication_host =
        select_ids_duplication_host(sp->serving_tac, session_ref.second);
  ...
  Logger::smf_app().info(
      "Handled IDS traffic influence notification active=%s; updated %u PDU "
      "session state records",
      active ? "true" : "false", updated_sessions);
}
