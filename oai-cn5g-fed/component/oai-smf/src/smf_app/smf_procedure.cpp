...
namespace {
bool resolve_ids_duplication_target(
    const std::string& host, struct in_addr& ipv4_address) {
  std::regex re_ipv4(oai::config::IPV4_ADDRESS_VALIDATOR_REGEX);
  if (std::regex_match(host, re_ipv4)) {
    ipv4_address = oai::utils::conv::fromString(host);
    return true;
  }
  ...
  uint8_t addr_type            = 0;
  try {
    if (!fqdn::resolve(host, resolved_address, resolved_port, addr_type)) {
      return false;
    }
  } catch (const std::runtime_error& err) {
    Logger::smf_app().warn(
        "Could not resolve IDS duplication target %s: %s", host.c_str(),
        err.what());
    return false;
  }
  ...
  if (addr_type != 0) {
    Logger::smf_app().warn(
        "IDS duplication target %s resolved to non-IPv4 address %s; skip "
        "duplication",
        host.c_str(), resolved_address.c_str());
    return false;
  }

  ipv4_address = oai::utils::conv::fromString(resolved_address);
  return true;
}
}  // namespace
...
pfcp::create_far smf_session_procedure::pfcp_create_far(
  ...
  n6_type.setEnumValue(UPInterfaceType_anyOf::eUPInterfaceType_anyOf::N6);
	  ...
	  if (edge->uplink && sps->ids_traffic_duplication_enabled) {
	    struct in_addr ids_ipv4_address = {};
	    if (resolve_ids_duplication_target(
	            sps->ids_traffic_duplication_host, ids_ipv4_address)) {
	      pfcp::duplicating_parameters duplicating_parameters = {};
	      pfcp::destination_interface_t duplicate_destination = {};
	      pfcp::outer_header_creation_t duplicate_outer_header = {};

	      apply_action.dupl = 1;
	      duplicate_destination.interface_value = pfcp::INTERFACE_VALUE_ACCESS;
	      duplicate_outer_header.outer_header_creation_description =
	          OUTER_HEADER_CREATION_GTPU_UDP_IPV4;
	      duplicate_outer_header.teid = sps->ids_traffic_duplication_teid;
	      duplicate_outer_header.ipv4_address = ids_ipv4_address;
	      duplicate_outer_header.port_number  = sps->ids_traffic_duplication_port;

	      duplicating_parameters.set(duplicate_destination);
	      duplicating_parameters.set(duplicate_outer_header);
	      create_far.set(duplicating_parameters);

	      Logger::smf_app().info(
	          "Added IDS duplication parameters to FAR %u: target=%s:%u teid=%u",
	          edge->far_id.far_id, sps->ids_traffic_duplication_host.c_str(),
	          sps->ids_traffic_duplication_port,
	          sps->ids_traffic_duplication_teid);
	    } else {
	      Logger::smf_app().warn(
	          "IDS duplication was requested, but target %s could not be "
	          "resolved. FAR %u keeps normal forwarding only.",
	          sps->ids_traffic_duplication_host.c_str(), edge->far_id.far_id);
	    }
	  }
...
smf_procedure_code session_create_sm_context_procedure::run(
  ...
  if (smf_app_inst->is_scid_2_smf_context(sm_context_req->scid)) {
    ...
    if (sp->policy_ptr && !smf_cfg->use_local_pcc_rules) {
    ...
    } else if (sp->policy_ptr && smf_cfg->use_local_pcc_rules) {
      Logger::smf_app().info(
          "Local PCC rules are enabled. Keeping default UPF selection and using "
          "PCF decision only as IDS duplication gate.");
    }
    if (!graph) {
