...
constexpr auto DEFAULT_IDS_DUPLICATION_HOST = "oai-ids";
constexpr uint16_t DEFAULT_IDS_DUPLICATION_PORT = 2152;
constexpr uint32_t DEFAULT_IDS_DUPLICATION_TEID = 0x1D5;
...
uint16_t getenv_u16_or_default(const char* name, uint16_t fallback) {
  const char* value = std::getenv(name);
  if (!value || std::string(value).empty()) {
    return fallback;
  }
...
uint32_t getenv_u32_or_default(const char* name, uint32_t fallback) {
  const char* value = std::getenv(name);
  if (!value || std::string(value).empty()) {
    return fallback;
  }
...
bool parse_supi64_token(const std::string& token, supi64_t& supi64) {
  std::string value = token;
  boost::algorithm::trim(value);
  if (boost::algorithm::starts_with(value, "imsi-")) {
    value.erase(0, 5);
  }
  if (value.empty() ||
  ...
  try {
    supi64 = static_cast<supi64_t>(std::stoull(value));
    return true;
  } catch (const std::exception&) {
    return false;
  }
...
std::string normalize_tac_token(const std::string& token) {
  std::string value = token;
  boost::algorithm::trim(value);
  if (boost::algorithm::istarts_with(value, "0x")) {
    value.erase(0, 2);
  }
...
bool select_ids_duplication_host_for_tac(
    const std::string& tac, std::string& host) {
  const std::string normalized_tac = normalize_tac_token(tac);
  if (normalized_tac.empty()) {
    return false;
  }

  const char* raw_rules = std::getenv("OAI_SMF_IDS_DUPLICATION_HOST_BY_TAC");
  if (!raw_rules || std::string(raw_rules).empty()) {
    return false;
  }

  std::vector<std::string> rules;
  const std::string raw(raw_rules);
  boost::algorithm::split(rules, raw, boost::algorithm::is_any_of(",;"));
  for (auto rule : rules) {
    ...
    std::string rule_tac  = rule.substr(0, separator);
    std::string rule_host = rule.substr(separator + 1);
    boost::algorithm::trim(rule_host);
    if (normalize_tac_token(rule_tac) == normalized_tac &&
        !rule_host.empty()) {
      host = rule_host;
      return true;
    }
...
std::string select_ids_duplication_host_for_supi(const supi64_t supi64) {
  const std::string fallback = getenv_or_default(
      "OAI_SMF_IDS_DUPLICATION_HOST", DEFAULT_IDS_DUPLICATION_HOST);
  const char* raw_rules = std::getenv("OAI_SMF_IDS_DUPLICATION_HOST_BY_SUPI");
  if (!raw_rules || std::string(raw_rules).empty()) {
    return fallback;
  }
  ...
  for (auto rule : rules) {
    ...
    std::string range = rule.substr(0, separator);
    std::string host  = rule.substr(separator + 1);
    boost::algorithm::trim(range);
    boost::algorithm::trim(host);
    if (range.empty() || host.empty()) {
      continue;
    }
    ...
    if (dash == std::string::npos) {
      if (!parse_supi64_token(range, start)) {
        continue;
      }
      end = start;
    } else {
      if (!parse_supi64_token(range.substr(0, dash), start) ||
          !parse_supi64_token(range.substr(dash + 1), end)) {
        continue;
      }
    }

    if (start <= supi64 && supi64 <= end) {
      return host;
    }
  }
...
std::string select_ids_duplication_host(
    const std::string& tac, const supi64_t supi64) {
  std::string host = {};
  if (select_ids_duplication_host_for_tac(tac, host)) {
    Logger::smf_n7().info(
        "Selected IDS duplication target %s using UE location TAC %s",
        host.c_str(), tac.c_str());
    return host;
  }

  return select_ids_duplication_host_for_supi(supi64);
}

bool sm_policy_decision_has_ids_duplication_trigger(
    const oai::model::pcf::SmPolicyDecision& decision) {
  if (!decision.pccRulesIsSet() || !decision.traffContDecsIsSet()) {
    return false;
  }

  const auto pcc_rules        = decision.getPccRules();
  const auto traffic_controls = decision.getTraffContDecs();

  for (const auto& rule : pcc_rules) {
    const auto& pcc_rule = rule.second;
    const bool ids_rule =
        rule.first == IDS_DUPLICATION_RULE_ID ||
        pcc_rule.getPccRuleId() == IDS_DUPLICATION_RULE_ID;
    if (!ids_rule) {
      continue;
    }

    for (const auto& tc_id : pcc_rule.getRefTcData()) {
      const auto traffic_it = traffic_controls.find(tc_id);
      if (traffic_it != traffic_controls.end() &&
          traffic_it->second.traffCorreIndIsSet() &&
          traffic_it->second.isTraffCorreInd()) {
        return true;
      }
    }
  }
...
void smf_context::handle_pdu_session_create_sm_context_request(
  ...
  if (smreq->req.serving_tac_is_set()) {
    sp->serving_tac = smreq->req.get_serving_tac();
    Logger::smf_app().debug(
        "Stored UE location TAC %s for PDU session %u",
        sp->serving_tac.c_str(), sp->pdu_session_id);
  }
  ...
  sp->policy_ptr->context.setNotificationUri(
      smf_cfg->local().get_sbi().get_url() +
      "/npcf-smpolicycontrol/v1/ids-notify");
  ...
  if (smf_cfg->use_local_pcc_rules) {
    ...
    sp->ids_traffic_duplication_enabled =
        n7::smf_n7::get_instance().ids_traffic_influence_active();
    if (sp->ids_traffic_duplication_enabled) {
      sp->ids_traffic_duplication_host =
          select_ids_duplication_host(sp->serving_tac, supi64);
      sp->ids_traffic_duplication_port = getenv_u16_or_default(
          "OAI_SMF_IDS_DUPLICATION_PORT", DEFAULT_IDS_DUPLICATION_PORT);
      sp->ids_traffic_duplication_teid = getenv_u32_or_default(
          "OAI_SMF_IDS_DUPLICATION_TEID", DEFAULT_IDS_DUPLICATION_TEID);
      Logger::smf_n7().info(
          "IDS traffic duplication enabled from passive PCF state for PDU "
          "session %u: target=%s:%u teid=%u",
          sp->pdu_session_id, sp->ids_traffic_duplication_host.c_str(),
          sp->ids_traffic_duplication_port, sp->ids_traffic_duplication_teid);
    } else {
      Logger::smf_n7().info(
          "Local PCC/default rules active and IDS traffic influence is "
          "inactive. Continue without PCF session policy.");
      sp->policy_ptr.reset();
    }
  } else {
    n7::sm_policy_status_code status =
	  ...
	  if (status != n7::sm_policy_status_code::CREATED) {
	    Logger::smf_n7().info(
	        ...
	        "using default rules");
	    use_pcf_policy = false;
	    sp->ids_traffic_duplication_enabled = false;
	    sp->policy_ptr.reset();
	    ...
	    use_pcf_policy = true;
	    sp->ids_traffic_duplication_enabled =
	        sm_policy_decision_has_ids_duplication_trigger(
	            sp->policy_ptr->decision);
	    if (sp->ids_traffic_duplication_enabled) {
	      sp->ids_traffic_duplication_host =
	          select_ids_duplication_host(sp->serving_tac, supi64);
	      sp->ids_traffic_duplication_port = getenv_u16_or_default(
	      ...
	      sp->ids_traffic_duplication_teid = getenv_u32_or_default(
	          "OAI_SMF_IDS_DUPLICATION_TEID", DEFAULT_IDS_DUPLICATION_TEID);
	      Logger::smf_n7().info(
	          "IDS traffic duplication gate enabled for PDU session %u: "
	          "target=%s:%u teid=%u",
	          sp->pdu_session_id, sp->ids_traffic_duplication_host.c_str(),
	          sp->ids_traffic_duplication_port,
	          sp->ids_traffic_duplication_teid);
	    } else {
	      Logger::smf_n7().info(
	          "PCF policy association created, but IDS duplication trigger was "
	          "not present. Continue with normal local/default rules.");
	      if (smf_cfg->use_local_pcc_rules) {
	        sp->policy_ptr.reset();
	      }
	    }
...
bool smf_context::handle_pdu_session_update_sm_context_request(
  ...
  if (sm_context_req_msg.serving_tac_is_set()) {
    sp->serving_tac = sm_context_req_msg.get_serving_tac();
    if (sp->ids_traffic_duplication_enabled) {
      const supi64_t supi64 = smf_supi_to_u64(smreq->req.get_supi());
      sp->ids_traffic_duplication_host =
          select_ids_duplication_host(sp->serving_tac, supi64);
    }
    Logger::smf_app().debug(
        "Updated UE location TAC %s for PDU session %u",
        sp->serving_tac.c_str(), sp->pdu_session_id);
  }
