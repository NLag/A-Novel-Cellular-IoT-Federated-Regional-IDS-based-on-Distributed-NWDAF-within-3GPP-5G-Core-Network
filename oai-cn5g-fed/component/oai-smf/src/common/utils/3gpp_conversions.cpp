...
namespace {
std::string tac_from_user_location(
    const oai::model::common::UserLocation& location) {
  if (location.nrLocationIsSet()) {
    return location.getNrLocation().getTai().getTac();
  }
  if (location.eutraLocationIsSet()) {
    return location.getEutraLocation().getTai().getTac();
  }
  return {};
}
}  // namespace
...
void xgpp_conv::sm_context_create_from_openapi(
  ...
  if (context_data.ueLocationIsSet()) {
    const std::string tac = tac_from_user_location(context_data.getUeLocation());
    if (!tac.empty()) {
      Logger::smf_app().debug("UE location TAC %s", tac.c_str());
      pcr.set_serving_tac(tac);
    }
  }
...
void xgpp_conv::sm_context_update_from_openapi(
  ...
  if (context_data.ueLocationIsSet()) {
    ...
    if (!tac.empty()) {
      Logger::smf_app().debug("UE location TAC %s", tac.c_str());
      pur.set_serving_tac(tac);
    }
  } else if (context_data.addUeLocationIsSet()) {
    const std::string tac =
        tac_from_user_location(context_data.getAddUeLocation());
    if (!tac.empty()) {
      Logger::smf_app().debug("Additional UE location TAC %s", tac.c_str());
      pur.set_serving_tac(tac);
    }
  }
