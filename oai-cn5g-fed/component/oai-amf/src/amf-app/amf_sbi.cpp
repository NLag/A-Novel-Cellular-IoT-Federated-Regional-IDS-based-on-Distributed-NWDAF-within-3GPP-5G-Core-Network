...
namespace {
std::string tac_to_openapi_string(uint32_t tac) {
  std::ostringstream stream;
  stream << std::hex << std::nouppercase << std::setw(6) << std::setfill('0')
         << (tac & 0x00ffffff);
  return stream.str();
}

nlohmann::json build_ue_location_json(
    const std::shared_ptr<ue_context>& uc) {
  nlohmann::json user_location = {};
  if (!uc) {
    return user_location;
  }

  const std::string mcc = uc->tai.mcc.empty() ? uc->cgi.mcc : uc->tai.mcc;
  const std::string mnc = uc->tai.mnc.empty() ? uc->cgi.mnc : uc->tai.mnc;
  const std::string tac = tac_to_openapi_string(uc->tai.tac);

  user_location["nrLocation"]["tai"]["plmnId"]["mcc"] = mcc;
  user_location["nrLocation"]["tai"]["plmnId"]["mnc"] = mnc;
  user_location["nrLocation"]["tai"]["tac"]           = tac;
  user_location["nrLocation"]["ncgi"]["plmnId"]["mcc"] = uc->cgi.mcc;
  user_location["nrLocation"]["ncgi"]["plmnId"]["mnc"] = uc->cgi.mnc;
  user_location["nrLocation"]["ncgi"]["nrCellId"] =
      std::to_string(uc->cgi.nrCellId);
...
void amf_sbi::send_pdu_session_update_sm_context_request(
  ...
  if (amf_app_inst->supi_2_ue_context(supi, uc)) {
    pdu_session_update_request["ueLocation"] = build_ue_location_json(uc);
  }
  std::string json_part = pdu_session_update_request.dump();
...
void amf_sbi::handle_pdu_session_initial_request(
  ...
  std::shared_ptr<ue_context> uc = {};
  if (amf_app_inst->supi_2_ue_context(supi, uc)) {
    pdu_session_establishment_request["ueLocation"] =
        build_ue_location_json(uc);
  }
