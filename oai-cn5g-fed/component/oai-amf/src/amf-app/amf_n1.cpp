...
static void log_illegal_ue_reject_diag(
    const char* site, const char* reason, uint32_t ran_ue_ngap_id,
    uint64_t amf_ue_ngap_id, const std::shared_ptr<nas_context>& nc) {
  const std::string imsi = nc ? nc->imsi : "";
  const std::string supi =
      (nc && !nc->imsi.empty()) ? amf_conv::imsi_to_supi(nc->imsi) : "";
  Logger::amf_n1().error(
      "IDS-LAB-DIAG Illegal UE reject: site=%s reason=%s "
      "ran_ue_ngap_id=" GNB_UE_NGAP_ID_FMT
      " amf_ue_ngap_id=" AMF_UE_NGAP_ID_FMT
      " imsi=%s supi=%s ctx_available=%s auth_vectors_present=%s",
      site, reason, ran_ue_ngap_id, amf_ue_ngap_id, imsi.c_str(),
      supi.c_str(),
      (nc && nc->ctx_avaliability_ind) ? "true" : "false",
      (nc && nc->is_auth_vectors_present) ? "true" : "false");
}
...
bool amf_n1::registration_request_handle(
  ...
  std::shared_ptr<ue_context> uc = {};
  if (!find_ue_context(ran_ue_ngap_id, amf_ue_ngap_id, uc)) {
    log_illegal_ue_reject_diag(
        "registration_request_handle", "find_ue_context_failed",
        ran_ue_ngap_id, amf_ue_ngap_id, nc);
  ...
  if (nc == nullptr) {
    ...
    if (guti_2_nas_context(guti, nc)) {
      ...
      uint32_t gnb_id = uc ? uc->gnb_id : 0;
      if (uc) uc.reset();
      ...
      if (!amf_n2_inst->ran_ue_id_2_ue_ngap_context(
        ...
        log_illegal_ue_reject_diag(
            "registration_request_handle", "guti_ngap_context_lookup_failed",
            ran_ue_ngap_id, amf_ue_ngap_id, nc);
        cause = k5gmmCauseIllegalUe;  // TODO: verify the cause
        return false;
      }
...
bool amf_n1::run_registration_procedure(
  ...
  if (!nc->ctx_avaliability_ind) {
    ...
    log_illegal_ue_reject_diag(
        "run_registration_procedure", "nas_context_not_available",
        nc->ran_ue_ngap_id, nc->amf_ue_ngap_id, nc);
    cause = k5gmmCauseIllegalUe;  // TODO: verify the cause
    return false;
  }
  ...
  if (nc->is_imsi_present) {
    ...
    if (!nc->is_auth_vectors_present) {
      ...
      if (auth_vectors_generator(nc)) {
        ...
        log_illegal_ue_reject_diag(
            "run_registration_procedure", "auth_vectors_generation_failed",
            nc->ran_ue_ngap_id, nc->amf_ue_ngap_id, nc);
        cause = k5gmmCauseIllegalUe;  // TODO: verify the cause
        return false;
      }
...
void amf_n1::authentication_response_handle(
  ...
  if (!amf_ue_id_2_nas_context(amf_ue_ngap_id, nc)) {
    log_illegal_ue_reject_diag(
        "authentication_response_handle", "nas_context_lookup_failed",
        ran_ue_ngap_id, amf_ue_ngap_id, nc);
    send_registration_reject_msg(
        ran_ue_ngap_id, amf_ue_ngap_id,
  ...
  if (!isAuthOk) {
    Logger::amf_n1().error(
        ...
        amf_ue_ngap_id);
    log_illegal_ue_reject_diag(
        "authentication_response_handle", "authentication_validation_failed",
        ran_ue_ngap_id, amf_ue_ngap_id, nc);
    send_registration_reject_msg(
        ran_ue_ngap_id, amf_ue_ngap_id,
        k5gmmCauseIllegalUe);  // cause?
...
void amf_n1::authentication_failure_handle(
  ...
  if (!amf_ue_id_2_nas_context(amf_ue_ngap_id, nc)) {
    log_illegal_ue_reject_diag(
        "authentication_failure_handle", "nas_context_lookup_failed",
        ran_ue_ngap_id, amf_ue_ngap_id, nc);
    send_registration_reject_msg(
        ran_ue_ngap_id, amf_ue_ngap_id,
        k5gmmCauseIllegalUe);  // cause?
    ...
    case k5gmmCauseSynchFailure: {
      ...
      if (auth_vectors_generator(nc)) {
        ...
        log_illegal_ue_reject_diag(
            ...
            "sync_failure_auth_vectors_generation_failed", nc->ran_ue_ngap_id,
            nc->amf_ue_ngap_id, nc);
        send_registration_reject_msg(
            nc->ran_ue_ngap_id, nc->amf_ue_ngap_id,
            k5gmmCauseIllegalUe);  // cause?
      }
...
bool amf_n1::run_mobility_registration_update_procedure(
  ...
  if (!find_ue_context(nc, uc)) {
    log_illegal_ue_reject_diag(
        ...
        "find_ue_context_failed", nc->ran_ue_ngap_id, nc->amf_ue_ngap_id, nc);
    cause = k5gmmCauseIllegalUe;  // TODO: verify the cause
    return false;
  }
...
bool amf_n1::run_periodic_registration_update_procedure(
  ...
  if (!find_ue_context(nc, uc)) {
    log_illegal_ue_reject_diag(
        "run_periodic_registration_update_procedure_experimental",
        "find_ue_context_failed", nc->ran_ue_ngap_id, nc->amf_ue_ngap_id, nc);
    cause = k5gmmCauseIllegalUe;  // TODO: verify the cause
    return false;
  }
