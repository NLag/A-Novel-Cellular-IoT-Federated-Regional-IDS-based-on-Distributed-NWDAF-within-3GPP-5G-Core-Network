          ...
          if (authorized_network_slice_info.find("nsiInformation") !=
            ...
            try {
              from_json(
                  ...
                  nsi_information);
              std::string nrf_uri = nsi_information.getNrfNfMgtUri();
              if (!nrf_uri.empty()) {
                const std::string instance_suffix = "/" + amf_instance_id;
                if ((nrf_uri.size() < instance_suffix.size()) ||
                    (nrf_uri.compare(
                         nrf_uri.size() - instance_suffix.size(),
                         instance_suffix.size(), instance_suffix) != 0)) {
                  if (nrf_uri.back() != '/') nrf_uri += "/";
                  nrf_uri += amf_instance_id;
                }
                nrfs.insert(nrf_uri);
              }
            } catch (std::exception& e) {
