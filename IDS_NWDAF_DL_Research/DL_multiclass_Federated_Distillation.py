...
if __name__ == "__main__":
    ...
    print("="*50)
    hidden_dim = 256  # Example hidden dimension, can be adjusted
    # Initialize MLP models
    model_list=[]
    for i in range(NUM_REGION):
        model = MLPClassifier(input_features=PACKET_LEN, output_dim=NUM_CLASS, hidden_dim=hidden_dim)
        model.to(DEVICE)
        model_list.append(model)
    
    # Run the federated learning process with quiet=QUIET
    round_metrics = run_federated_distillation(
        model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="MLP"
    )

    # Save round metrics dictionary to a file
    with open('round_metrics_FD_MLP.pkl', 'wb') as f:
        pickle.dump(round_metrics, f)

    # After all rounds, plot the convergence metrics
    plot_progress(round_metrics, image_path='convergence_plots_FD_MLP.png')
    
    all_mlp_test_results = []
    all_mlp_eval_results = []

    # Test the final models on the test dataset
    print("Testing final models on test dataset")
    for model_idx,model in enumerate(model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"MLP_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_MLP_{model_idx}_test.png")
        all_mlp_test_results.append(test_results)

    # Test the final model on the evaluation dataset
    print("Testing final models on evaluation dataset")
    for model_idx,model in enumerate(model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"MLP_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_MLP_{model_idx}_eval.png")
        all_mlp_eval_results.append(eval_results)

    # Save test results, and evaluation results
    with open('private_test_results_FD_MLP.pkl', 'wb') as f:
        pickle.dump(all_mlp_test_results , f)
    with open('evaluation_results_FD_MLP.pkl', 'wb') as f:
        pickle.dump(all_mlp_eval_results, f)

    # Save models
    for model_idx, model in enumerate(model_list):
        torch.save(model.state_dict(), f'model_FD_MLP_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_MLP_{model_idx+1}.pth")
    ...
    cnn_round_metrics = run_federated_distillation(
        cnn_model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="CNN"
    )

    # Save round metrics dictionary to a file
    with open('round_metrics_FD_CNN.pkl', 'wb') as f:
        pickle.dump(cnn_round_metrics, f)
    
    plot_progress(cnn_round_metrics, image_path='convergence_plots_FD_CNN.png')
    
    # Initialize lists to collect results from all CNN models
    all_cnn_test_results = []
    all_cnn_eval_results = []
    
    # Test the final models on the test dataset
    print("Testing CNN final models on test dataset")
    for model_idx, model in enumerate(cnn_model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"CNN_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_CNN_{model_idx}_test.png")
        all_cnn_test_results.append(test_results)

    # Test the final models on the evaluation dataset
    print("Testing CNN final models on evaluation dataset")
    for model_idx, model in enumerate(cnn_model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"CNN_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_CNN_{model_idx}_eval.png")
        all_cnn_eval_results.append(eval_results)

    # Save test results, and evaluation results
    with open('private_test_results_FD_CNN.pkl', 'wb') as f:
        pickle.dump(all_cnn_test_results, f)
    with open('evaluation_results_FD_CNN.pkl', 'wb') as f:
        pickle.dump(all_cnn_eval_results, f)

    # Save models
    for model_idx, model in enumerate(cnn_model_list): 
        torch.save(model.state_dict(), f'model_FD_CNN_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_CNN_{model_idx+1}.pth")
    ...
    rnn_round_metrics = run_federated_distillation(
        rnn_model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="RNN"
    )

    # Save round metrics dictionary to a file
    with open('round_metrics_FD_RNN.pkl', 'wb') as f:
        pickle.dump(rnn_round_metrics, f)
    
    plot_progress(rnn_round_metrics, image_path='convergence_plots_FD_RNN.png')
    
    # Initialize lists to collect results from all RNN models
    all_rnn_test_results = []
    all_rnn_eval_results = []
    
    # Test the final models on the test dataset
    print("Testing RNN final models on test dataset")
    for model_idx, model in enumerate(rnn_model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"RNN_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_RNN_{model_idx}_test.png")
        all_rnn_test_results.append(test_results)

    # Test the final models on the evaluation dataset
    print("Testing RNN final models on evaluation dataset")
    for model_idx, model in enumerate(rnn_model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"RNN_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_RNN_{model_idx}_eval.png")
        all_rnn_eval_results.append(eval_results)
    
    # Save test results, and evaluation results
    with open('private_test_results_FD_RNN.pkl', 'wb') as f:
        pickle.dump(all_rnn_test_results, f)
    with open('evaluation_results_FD_RNN.pkl', 'wb') as f:
        pickle.dump(all_rnn_eval_results, f)
        
    # Save models
    for model_idx, model in enumerate(rnn_model_list):
        torch.save(model.state_dict(), f'model_FD_RNN_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_RNN_{model_idx+1}.pth")
    ...
    gru_round_metrics = run_federated_distillation(
        gru_model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="GRU"
    )
    
    # Save round metrics dictionary to a file
    with open('round_metrics_FD_GRU.pkl', 'wb') as f:
        pickle.dump(gru_round_metrics, f)

    plot_progress(gru_round_metrics, image_path='convergence_plots_FD_GRU.png')
    
    # Initialize lists to collect results from all GRU models
    all_gru_test_results = []
    all_gru_eval_results = []
    
    # Test the final models on the test dataset
    print("Testing GRU final models on test dataset")
    for model_idx, model in enumerate(gru_model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"GRU_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_GRU_{model_idx}_test.png")
        all_gru_test_results.append(test_results)

    # Test the final models on the evaluation dataset
    print("Testing GRU final models on evaluation dataset")
    for model_idx, model in enumerate(gru_model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"GRU_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_GRU_{model_idx}_eval.png")
        all_gru_eval_results.append(eval_results)

    # Save test results, and evaluation results
    with open('private_test_results_FD_GRU.pkl', 'wb') as f:
        pickle.dump(all_gru_test_results, f)
    with open('evaluation_results_FD_GRU.pkl', 'wb') as f:
        pickle.dump(all_gru_eval_results, f)
    
    # Save models
    for model_idx, model in enumerate(gru_model_list):
        torch.save(model.state_dict(), f'model_FD_GRU_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_GRU_{model_idx+1}.pth")
    ...
    lstm_model_list=[]
    for i in range(NUM_REGION):
        model = LSTMClassifier(input_features=PACKET_LEN, hidden_dim=256, output_dim=NUM_CLASS)
        model.to(DEVICE)
        lstm_model_list.append(model)
    
    lstm_round_metrics = run_federated_distillation(
        lstm_model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="LSTM"
    )
    
    # Save round metrics dictionary to a file
    with open('round_metrics_FD_LSTM.pkl', 'wb') as f:
        pickle.dump(lstm_round_metrics, f)

    plot_progress(lstm_round_metrics, image_path='convergence_plots_FD_LSTM.png')
    
    # Initialize lists to collect results from all LSTM models
    all_lstm_test_results = []
    all_lstm_eval_results = []
    
    # Test the final models on the test dataset
    print("Testing LSTM final models on test dataset")
    for model_idx, model in enumerate(lstm_model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"LSTM_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_LSTM_{model_idx}_test.png")
        all_lstm_test_results.append(test_results)

    # Test the final models on the evaluation dataset
    print("Testing LSTM final models on evaluation dataset")
    for model_idx, model in enumerate(lstm_model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"LSTM_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_LSTM_{model_idx}_eval.png")
        all_lstm_eval_results.append(eval_results)

    with open('private_test_results_FD_LSTM.pkl', 'wb') as f:
        pickle.dump(all_lstm_test_results, f)
    with open('evaluation_results_FD_LSTM.pkl', 'wb') as f:
        pickle.dump(all_lstm_eval_results, f)

    # Save models
    for model_idx, model in enumerate(lstm_model_list):
        torch.save(model.state_dict(), f'model_FD_LSTM_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_LSTM_{model_idx+1}.pth")
    ...
    transformer_round_metrics = run_federated_distillation(
        transformer_model_list, public_dataloaders, train_loaders, validation_loaders, test_loaders, 
        rounds=ROUNDS, quiet=QUIET, debug=DEBUG, model_name="Transformer"
    )
    
    # Save round metrics dictionary to a file
    with open('round_metrics_FD_Transformer.pkl', 'wb') as f:
        pickle.dump(transformer_round_metrics, f)

    plot_progress(transformer_round_metrics, image_path='convergence_plots_FD_Transformer.png')
    
    # Initialize lists to collect results from all Transformer models
    all_transformer_test_results = []
    all_transformer_eval_results = []
    
    # Test the final models on the test dataset
    print("Testing Transformer final models on test dataset")
    for model_idx, model in enumerate(transformer_model_list):
        print(f"Model {model_idx}:")
        # Evaluate the model on the private test dataset
        test_results = evaluate_model(model, test_loaders, quiet=QUIET)
        print_results(test_results, cmfig_title=f"Transformer_{model_idx} Private Test Confusion Matrix", cmfig_filename=f"confusion_matrix_Transformer_{model_idx}_test.png")
        all_transformer_test_results.append(test_results)

    # Test the final models on the evaluation dataset
    print("Testing Transformer final models on evaluation dataset")
    for model_idx, model in enumerate(transformer_model_list):
        print(f"Model {model_idx}:")
        eval_results = evaluate_model(model, [eval_loader], quiet=QUIET)
        print_results(eval_results, cmfig_title=f"Transformer_{model_idx} Evaluation Confusion Matrix", cmfig_filename=f"confusion_matrix_Transformer_{model_idx}_eval.png")
        all_transformer_eval_results.append(eval_results)
    
    # Save test results, and evaluation results
    with open('private_test_results_FD_Transformer.pkl', 'wb') as f:
        pickle.dump(all_transformer_test_results, f)
    with open('evaluation_results_FD_Transformer.pkl', 'wb') as f:
        pickle.dump(all_transformer_eval_results, f)

    # Save models
    for model_idx, model in enumerate(transformer_model_list):
        torch.save(model.state_dict(), f'model_FD_Transformer_{model_idx+1}.pth')
        print(f"Model {model_idx} saved as model_FD_Transformer_{model_idx+1}.pth")

    print("\n" + "="*50)
    print("All models trained and evaluated.")

    # Create a new directory and move all results files there
    results_dir = './result0707_savedmodels'
    ...
    # Move all result files to the new directory
    for filename in os.listdir('.'):
        if (filename.startswith('round_metrics_FD_') or 
            filename.startswith('private_test_results_FD_') or 
            filename.startswith('evaluation_results_FD_') or 
            filename.startswith('convergence_plots_FD_') or 
            filename.startswith('confusion_matrix_') or 
            filename.startswith('model_FD_')):
            os.rename(filename, os.path.join(results_dir, filename))
