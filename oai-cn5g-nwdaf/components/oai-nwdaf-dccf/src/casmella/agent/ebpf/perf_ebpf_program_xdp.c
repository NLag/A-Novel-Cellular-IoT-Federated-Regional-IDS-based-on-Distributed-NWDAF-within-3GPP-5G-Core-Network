...
BPF_PERF_OUTPUT(http2_events);
BPF_PERF_OUTPUT(pfcp_events);
BPF_PERF_OUTPUT(ngap_events);
BPF_PERF_OUTPUT(gtpu_events);
...
int trafic_handler(struct xdp_md *ctx)
{
  ...
  if ( ((void*)eth + sizeof(*eth) <= data_end) && (eth->h_proto == htons(ETH_P_IP)) ) {
      ...
      if ( (void*)tcp + sizeof(*tcp) <= data_end ) {
        ...
        if(payload_length >= 9){
          ...
          if( (data_length == payload_length-9) && (frame_type == 1) ){
            ...
            // copy raw bytes
            int ret = bpf_probe_read(&http2_event->raw, raw_size & (MAX_RAW_SIZE-1), (unsigned char *)raw_offset);

            // send the event
            http2_events.perf_submit(ctx, http2_event, event_size & (MAX_RAW_SIZE-1));

            return XDP_PASS;

            /* OLD
            struct evt http2_event = {};
            http2_event.ip_src = ip->saddr;
            http2_event.ip_dest = ip->daddr;
            http2_event.src_port = ntohs(tcp->source);
            http2_event.dst_port = ntohs(tcp->dest);
            http2_event.payload_length = payload_length;
            http2_event.payload_offset = payload_offset;
            http2_event.capturing_time = capturing_time;

            http2_events.perf_submit_skb(ctx, len, &http2_event, sizeof(http2_event));
            return XDP_PASS;
            */
        ...
        return XDP_PASS; // TCP but not HTTP2
      }
    ...
    if (ip->protocol == IPPROTO_SCTP) {
      ...
      struct sctphdr *sctp = (void*)ip + sizeof(*ip);
    ...
    if ( (void*)sctp + sizeof(*sctp) <= data_end ) {
      ...
      if ( ((void*)sctpchunk + sizeof(*sctpchunk) <= data_end) ) {
            ...
            // copy raw bytes
            int ret = bpf_probe_read(&ngap_event->raw, raw_size & (MAX_RAW_SIZE-1), (unsigned char *)raw_offset);

            // send the event
            ngap_events.perf_submit(ctx, ngap_event, event_size & (MAX_RAW_SIZE-1));
    ...
    if ( (void*)udp + sizeof(*udp) <= data_end ) {
      ...
      if ( (udp->dest == htons(2152)) || (udp->source == htons(2152)) ) {
            ...
            unsigned int raw_size = (unsigned int)find_min((u32)GTPU_PAYLOAD_SIZE, (u32)MAX_RAW_SIZE);
            if (raw_size <= 0){
              return XDP_PASS;
            }
            u64 event_size = (u64)BASE_EVENT_SIZE + raw_size;
            ...
            // copy raw bytes
            int ret = bpf_probe_read(&gtpu_event->raw, raw_size & (MAX_RAW_SIZE-1), (unsigned char *)raw_offset);

            // send the event
            gtpu_events.perf_submit(ctx, gtpu_event, event_size & (MAX_RAW_SIZE-1));

            return XDP_PASS;

            /* OLD
            struct evt gtpu_event = {};
            gtpu_event.ip_src = ip->saddr;
            gtpu_event.ip_dest = ip->daddr;
            gtpu_event.src_port = ntohs(udp->source);
            gtpu_event.dst_port = ntohs(udp->dest);
            gtpu_event.payload_length = ntohs(udp->len) - sizeof(*udp);
            gtpu_event.payload_offset = sizeof(*eth) + ip_header_length + sizeof(*udp);
            gtpu_event.capturing_time = capturing_time;

            // gtpu_events.perf_submit(ctx, &gtpu_event, sizeof(gtpu_event));
            gtpu_events.perf_submit_skb(ctx, len, &gtpu_event, sizeof(gtpu_event));
      ...
      if ( (udp->dest == htons(8805)) || (udp->source == htons(8805)) ) {
            ...
            // copy raw bytes
            int ret = bpf_probe_read(&pfcp_event->raw, raw_size & (MAX_RAW_SIZE-1), (unsigned char *)raw_offset);

            // send the event
            pfcp_events.perf_submit(ctx, pfcp_event, event_size & (MAX_RAW_SIZE-1));

            return XDP_PASS;

            /* OLD
            struct evt pfcp_event = {};
            pfcp_event.ip_src = ip->saddr;
            pfcp_event.ip_dest = ip->saddr;
            pfcp_event.src_port = ntohs(udp->source);
            pfcp_event.dst_port = ntohs(udp->dest);
            pfcp_event.payload_length = ntohs(udp->len) - sizeof(*udp);
            pfcp_event.payload_offset = sizeof(*eth) + ip_header_length + sizeof(*udp);
            pfcp_event.capturing_time = capturing_time;

            pfcp_events.perf_submit_skb(ctx, len, &pfcp_event, sizeof(pfcp_event));
    ...
    return XDP_PASS; // UDP but nor GTP-U nor PFCP
