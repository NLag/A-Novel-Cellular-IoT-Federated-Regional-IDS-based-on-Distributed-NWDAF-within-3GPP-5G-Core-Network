...
int trafic_handler(struct __sk_buff *skb, u64 capturing_time, __u8 layer)
{
  ...
  if ( ((void*)eth + sizeof(*eth) <= data_end) && (eth->h_proto == htons(ETH_P_IP)) ) {
      ...
      if ( (void*)tcp + sizeof(*tcp) <= data_end ) {
        ...
        if(payload_length >= 9){
          ...
          if( (data_length == payload_length-9) && (frame_type == 1) ){
            ...
            // send the event
            http2_events.perf_submit(skb, http2_event, event_size & (MAX_RAW_SIZE-1));
            return TC_ACT_OK;
            ...
            http2_events.perf_submit_skb(skb, skb->len, &http2_event, sizeof(http2_event));
            return TC_ACT_OK;
            */
        ...
        return TC_ACT_OK; // TCP but nor HTTP nor HTTP2
      }
    ...
    if ( (void*)udp + sizeof(*udp) <= data_end ) {
      ...
      if ( (udp->dest == htons(2152)) || (udp->source == htons(2152)) ) {
            ...
            // send the event
            gtpu_events.perf_submit(skb, gtpu_event, event_size & (MAX_RAW_SIZE-1));
            ...
            //gtpu_events.perf_submit(skb, &gtpu_event, sizeof(gtpu_event));
            gtpu_events.perf_submit_skb(skb, skb->len, &gtpu_event, sizeof(gtpu_event));
            return TC_ACT_OK;
            */
      ...
      if ( (udp->dest == htons(8805)) || (udp->source == htons(8805)) ) {
            ...
            // send the event
            pfcp_events.perf_submit(skb, pfcp_event, event_size & (MAX_RAW_SIZE-1));
            ...
            pfcp_events.perf_submit_skb(skb, skb->len, &pfcp_event, sizeof(pfcp_event));
