...
type SbiConfig struct {
	...
	Pcf struct {
		IpAddr            string `envconfig:"PCF_IP_ADDR"`
		SubRoute          string `envconfig:"PCF_SUBSCR_ROUTE"`
		ApiRoute          string `envconfig:"PCF_API_ROUTE"`
		NotifCorrId       string `envconfig:"PCF_NOTIFY_CORRELATION_ID"`
		NotifId           string `envconfig:"PCF_NOTIFICATION_ID"`
		NorifForwardRoute string `envconfig:"PCF_NOTIFICATION_FORWARD_ROUTE"`
	}
	Nrf struct {
		Endpoint       string `envconfig:"NRF_ENDPOINT"`
		NFInstanceID   string `envconfig:"NRF_NF_INSTANCE_ID"`
		IPv4Address    string `envconfig:"NRF_IPV4_ADDRESS"`
		SBIPort        int    `envconfig:"NRF_SBI_PORT"`
		HeartbeatTimer int    `envconfig:"NRF_HEARTBEAT_TIMER"`
		AutoRegister   bool   `envconfig:"NRF_AUTO_REGISTER"`
	}
	Dccf struct {
		Endpoint string `envconfig:"DCCF_ENDPOINT"`
	}
	// Data collection mode: "DCCF_ONLY" to centralize all data collection through DCCF
	DataCollectionMode string `envconfig:"DATA_COLLECTION_MODE"`
	Database           struct {
		...
		CollectionPcfName string `envconfig:"MONGODB_COLLECTION_NAME_PCF"`
	}
	Server struct {
...
type accessTypeChange struct {
	AccType     pcf_client.AccessType
	RatType     pcf_client.RatType
	Snssai      *pcf_client.Snssai
	Dnn         *string
	PduSessInfo *pcf_client.PduSessionInformation
	TimeStamp   int64
}

type plmnChange struct {
	PlmnId      *pcf_client.PlmnIdNid
	Snssai      *pcf_client.Snssai
	Dnn         *string
	PduSessInfo *pcf_client.PduSessionInformation
	TimeStamp   int64
...
type genericPcfEvent struct {
	Notification pcf_client.PcEventNotification
	TimeStamp    int64
}
