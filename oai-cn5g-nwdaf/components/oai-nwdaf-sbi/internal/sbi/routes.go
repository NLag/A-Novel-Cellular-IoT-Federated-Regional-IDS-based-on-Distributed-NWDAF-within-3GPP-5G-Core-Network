...
func NewRouter() http.Handler {
	...
	// register routes for NF event notifications
	mux.HandleFunc(config.Amf.ApiRoute, storeAmfNotificationOnDB)
	mux.HandleFunc(config.Smf.ApiRoute, storeSmfNotificationOnDB)
	mux.HandleFunc(config.Pcf.ApiRoute, storePcfNotificationOnDB)
	// register route for DCCF notifications
	mux.HandleFunc("/dccf/notifications", DCCFNotificationHandler)
	return mux
}
